...
The CompletionProvider offers context-aware completions for Ansible playbooks, including keywords, module names, module options, and Jinja2 elements.

*** Data Flow

#+begin_example
Position (line, col) + trigger character
       |
       v
AnsibleDocument.get_completion_context()
       |
       v
CompletionContext (context, existing_keys, module_name, etc.)
       |
       v
CompletionProvider dispatches to handler
       |
       v
types.CompletionList (items with labels, kinds, documentation)
#+end_example

*** Implementation

#+begin_src python :tangle ../../src/ansible_ls/providers/completion_provider.py :mkdirp t
...
*** TODO Tests
:PROPERTIES:
:ID:       8b3f0981-c980-4f21-a8e8-b946bad23186
:END:

#+begin_src python :tangle ../../tests/providers/test_completion_provider.py :mkdirp t
