...
For Arch Linux users, a PKGBUILD enables system-wide installation via =makepkg=.
...
depends=(
    'python>=3.10'
    'python-pygls>=1.3.0'
    'python-ruamel-yaml>=0.18.0'
    'python-attrs>=23.0.0'
    'python-tree-sitter>=0.23.0'
    'python-lsprotocol>=2024.0.0'
    'python-ansible-runner>=2.4.0'
    'ansible>=10.0.0'
    'python-jinja>=3.1.0'
)
makedepends=(
...
optdepends=(
    'python-tree-sitter-yaml: YAML parsing support'
    'python-tree-sitter-jinja2: Jinja2 template support'
)
provides=('ansible-language-server')
conflicts=('nodejs-ansible-language-server')
source=()

# For local development, override source in makepkg.conf or use:
# makepkg -e  (to skip source extraction, build from current directory)

build() {
    cd "${srcdir}/.." || cd "${startdir}/../.."
...
For local development without rebuilding the package on each change:

#+begin_src shell :tangle no
# Install in editable mode system-wide (requires root)
sudo pip install -e .
...
For AUR submission, create a source-based PKGBUILD that fetches from git or PyPI:

#+begin_src bash :tangle no
# AUR-style PKGBUILD (example - adjust for actual release)
pkgname=python-ansible-language-server
pkgver=0.1.0
pkgrel=1
pkgdesc="Language Server Protocol implementation for Ansible (Python)"
...
url="https://github.com/ansible/ansible-language-server"
license=('GPL3')
depends=('python' 'python-pygls' 'python-ruamel-yaml' 'python-attrs'
         'python-tree-sitter' 'python-lsprotocol' 'python-ansible-runner'
         'ansible' 'python-jinja')
makedepends=('python-build' 'python-installer' 'python-hatchling')
source=("${pkgname}-${pkgver}.tar.gz::https://github.com/ansible/ansible-language-server/archive/v${pkgver}.tar.gz")
sha256sums=('SKIP')

build() {
    cd "ansible-language-server-${pkgver}"
    python -m build --wheel --no-isolation
}
