...
**** Arch Linux Package
:PROPERTIES:
:ID:       arch-linux-package
:END:

For Arch Linux installation, we avoid AUR dependency chains by bundling dependencies in an isolated virtual environment.

**Option 1: pipx (Recommended for Users)**

The simplest approach uses =pipx= which manages isolated environments automatically:
...
pipx install git+https://github.com/ansible/ansible-language-server.git

# The binary is available at ~/.local/bin/ansible-language-server
#+end_src

**Option 2: System Package with Bundled venv**

For system-wide installation, the PKGBUILD creates an isolated venv in =/opt= with all dependencies bundled via pip. This avoids requiring AUR packages like =python-pygls=, =python-lsprotocol=, etc.
...
#+begin_src bash :tangle ../pkg/arch/PKGBUILD :mkdirp t
...
#+begin_src bash :tangle ../pkg/arch/python-ansible-language-server.install :mkdirp t
post_install() {
    echo "ansible-language-server installed to /usr/bin/ansible-language-server"
    echo ""
    echo "Configure your editor to use the language server:"
    echo "  VS Code: Set ansible.languageServer.path to /usr/bin/ansible-language-server"
    echo "  Emacs:   (add-to-list 'eglot-server-programs '(yaml-mode . (\"ansible-language-server\")))"
}
...
#+begin_src shell :tangle no
# Create a local venv and install in editable mode
python -m venv .venv
