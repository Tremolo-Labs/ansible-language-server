...
#+begin_src dot :file images/architecture.png :cmdline -Kdot -Tpng :exports results
digraph architecture {
    ...
    subgraph cluster_client {
        ...
        style=dashed;
        color=gray;
        client [label="VS Code / Emacs / Neovim", fillcolor="#e3f2fd"];
    }
    ...
    subgraph cluster_server {
        ...
        pygls [label="pygls LanguageServer\n(JSON-RPC handler)", fillcolor="#fff3e0"];

        subgraph cluster_providers {
            label="Providers";
            style=filled;
            color="#fafafa";
            hover [label="HoverProvider", fillcolor="#e8f5e9", href="id:hover-provider"];
            completion [label="CompletionProvider", fillcolor="#e8f5e9", href="id:completion-provider"];
            definition [label="DefinitionProvider", fillcolor="#e8f5e9", href="id:definition-provider"];
        }

        subgraph cluster_services {
            label="Services (per workspace)";
            style=filled;
            color="#fafafa";
            settings [label="SettingsManager", fillcolor="#fce4ec", href="id:settings-service"];
            workspace [label="WorkspaceManager", fillcolor="#fce4ec", href="id:workspace-service"];
            docs [label="DocsLibrary", fillcolor="#fce4ec", href="id:docs-library"];
            validation [label="ValidationManager", fillcolor="#fce4ec", href="id:validation-service"];
            execenv [label="ExecutionEnvironment", fillcolor="#fce4ec", href="id:container-management"];
        }
        ...
        subgraph cluster_utils {
            ...
            style=filled;
            color="#fafafa";
            yaml [label="yaml_utils\n(tree-sitter)", fillcolor="#f3e5f5", href="id:yaml-parsing"];
            keywords [label="ansible_keywords\n(hybrid)", fillcolor="#f3e5f5", href="id:ansible-keywords"];
        }
    }

    client -> pygls [label="LSP\n(stdio/TCP)", style=bold];
    pygls -> hover [label="textDocument/hover"];
    pygls -> completion [label="textDocument/completion"];
    pygls -> definition [label="textDocument/definition"];
    ...
    settings -> workspace [style=dashed];
    workspace -> docs;
    workspace -> validation;
    workspace -> execenv;

    docs -> keywords [style=dotted];
    validation -> execenv [style=dotted, label="optional"];
}
#+end_src
...
#+begin_src dot :file images/dataflow.png :cmdline -Kdot -Tpng :exports results
digraph dataflow {
    rankdir=LR;
    node [shape=box, style="rounded,filled", fontname="Helvetica"];
    ...
    subgraph cluster_input {
        label="Input";
        doc [label="YAML Document", fillcolor="#e3f2fd"];
        pos [label="Cursor Position", fillcolor="#e3f2fd"];
    }

    subgraph cluster_parse {
        label="Parsing";
        tree [label="tree-sitter AST", fillcolor="#fff3e0"];
        path [label="Path to Node", fillcolor="#fff3e0"];
        ancestry [label="AncestryBuilder", fillcolor="#fff3e0", href="id:ancestry-builder"];
    }

    subgraph cluster_context {
        label="Context Detection";
        ctx [label="play | task | block | role", fillcolor="#e8f5e9"];
    }

    subgraph cluster_output {
        label="Output";
        kw [label="Keyword Docs", fillcolor="#fce4ec"];
        mod [label="Module Docs", fillcolor="#fce4ec"];
        comp [label="Completions", fillcolor="#fce4ec"];
    }

    doc -> tree [label="parse_yaml()"];
    pos -> path;
    tree -> path [label="get_path_to_position()"];
    path -> ancestry;
    ancestry -> ctx [label="parent().get_string_key()"];
    ctx -> kw;
    ctx -> mod;
    ctx -> comp;
}
...
#+begin_src dot :file images/grammar-composition.png :cmdline -Kdot -Tpng :exports results
digraph grammar {
    rankdir=TB;
    node [shape=box, style="rounded,filled", fontname="Helvetica"];
    ...
    subgraph cluster_base {
        label="Base Grammars";
        yaml [label="tree-sitter-yaml", fillcolor="#e3f2fd"];
        jinja [label="tree-sitter-jinja2", fillcolor="#fff3e0"];
    }

    subgraph cluster_ansible {
        label="Composite Grammar";
        style=filled;
        color="#f5f5f5";

        playbook [label="playbook", fillcolor="#e8f5e9"];
        play [label="play", fillcolor="#e8f5e9"];
        task [label="task", fillcolor="#e8f5e9"];
        block [label="block", fillcolor="#e8f5e9"];
        role_ref [label="role_reference", fillcolor="#e8f5e9"];
        template [label="jinja_template", fillcolor="#fce4ec"];
        var_ref [label="variable_reference", fillcolor="#fce4ec"];
    }

    yaml -> playbook [label="extends"];
    yaml -> play;
    yaml -> task;
    yaml -> block;
    jinja -> template [label="embeds"];
    jinja -> var_ref;

    playbook -> play [label="contains"];
    play -> task;
    play -> block;
    play -> role_ref;
    block -> task;
    task -> template [style=dashed, label="string values"];
}
#+end_src
...
#+begin_src dot :file images/build-pipeline.png :cmdline -Kdot -Tpng :exports results
digraph pipeline {
    rankdir=LR;
    node [shape=box, style="rounded,filled", fontname="Helvetica"];
    ...
    subgraph cluster_inputs {
        label="Inputs";
        ansible [label="Ansible Source\n(fattributes)", fillcolor="#e3f2fd"];
        yaml_grammar [label="tree-sitter-yaml\ngrammar.js", fillcolor="#e3f2fd"];
        jinja_grammar [label="tree-sitter-jinja2\ngrammar.js", fillcolor="#e3f2fd"];
    }

    subgraph cluster_generate {
        label="Generation (Python)";
        extract [label="extract_keywords.py", fillcolor="#fff3e0"];
        compose [label="compose_grammar.py", fillcolor="#fff3e0"];
    }

    subgraph cluster_build {
        label="Build (Node.js)";
        treesitter [label="tree-sitter generate", fillcolor="#e8f5e9"];
        compile [label="tree-sitter build", fillcolor="#e8f5e9"];
    }

    subgraph cluster_outputs {
        label="Outputs";
        parser_c [label="parser.c", fillcolor="#fce4ec"];
        bindings [label="Python bindings\n(.so/.pyd)", fillcolor="#fce4ec"];
    }

    ansible -> extract;
    extract -> compose [label="keywords.json"];
    yaml_grammar -> compose;
    jinja_grammar -> compose;
    compose -> treesitter [label="grammar.js"];
    treesitter -> parser_c;
    parser_c -> compile;
    compile -> bindings;
}
