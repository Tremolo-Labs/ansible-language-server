...
**** Dataclass Generator
:PROPERTIES:
:ID:       dataclass-generator
:END:

Generates Python dataclasses and validation sets from =build/ansible_metadata.json=. Run after the extraction script.

#+begin_src python :tangle tools/generate_types.py :mkdirp t
