...
See [[id:ancestry-builder][AncestryBuilder]] for navigation implementation.
...
When Ansible isn't installed, the extraction script falls back to static lists. These are maintained manually but should cover common use cases. The static fallback file extends [[file:src/ansible_ls/utils/ansible_keywords.py][ansible_keywords.py]] with Jinja2 filter and test definitions.

#+begin_src python :tangle src/ansible_ls/utils/jinja_fallbacks.py :mkdirp t
